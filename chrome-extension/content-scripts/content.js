...
//   sendResponse({nextGuess: next()})
// })

var boardContainer = document.querySelector("body > game-app").shadowRoot.querySelector("#board-container")
var board = document.querySelector("body > game-app").shadowRoot.querySelector("#board")

boardContainer.style.flexDirection = "column"

var wordleHelper = document.createElement("div")

var nextGuessButton = document.createElement("button")
nextGuessButton.innerText = "Get next guess"
nextGuessButton.style.paddingTop = nextGuessButton.style.paddingBottom = '0.5rem'
nextGuessButton.style.paddingLeft = nextGuessButton.style.paddingRight = '1rem'
nextGuessButton.style.marginRight = '0.5rem'

var nextGuessText = document.createElement("span");
nextGuessText.innerText = ""

nextGuessButton.onclick = function() {
  nextGuessText.innerText = next()
}

wordleHelper.appendChild(nextGuessButton)
wordleHelper.appendChild(nextGuessText)
boardContainer.insertBefore(wordleHelper, board)
...
function next() {
  processGameTiles()
  var reducedWordList = reduceWordList()
  return nextGuess(reducedWordList)
}
  ...
  reducedWordList.forEach(word => {
    ...
    word.split('').forEach((char, i) => {
      if(word.indexOf(char) >= i) {
        if (letters[char.charCodeAt() - 'a'.charCodeAt()] === null) {
          newLetters += 1
        }
